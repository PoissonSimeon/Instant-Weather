...
function createWeatherCard(city, forecast, options) {
  ...
  const title = `${city.name} - ${date.toLocaleDateString("fr-FR")}`;
  const description = getWeatherDescription(forecast.weather);
  const windDir = getWindDirection(forecast.wind10m.direction);

  let content = `<h3>${title}</h3><p>${description}</p>`;
  ...
  if (options.showDir) content += `<p>Direction du vent: ${windDir}</p>`;
